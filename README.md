...
An AI voice agent, **Maya**, answers every property enquiry in under a minute. She qualifies the buyer and then routes them: a hot lead goes live to a human agent, a warm lead gets an inspection booked, and a cold lead goes to nurture. Every step is logged.
...
flowchart LR
  ...
  I -->|WhatsApp chosen| W[Thank-you page:<br/>Talk to Maya on WhatsApp]
  I -->|Call me + consent| C[ElevenLabs outbound call]
  I -->|calling unavailable| A[Alert to the team]
  W -->|lead taps call| M((Maya<br/>ElevenLabs agent))
  C --> M
...
2. **First contact in under 60 seconds.** Either the lead taps *Talk to Maya on WhatsApp* and calls her, or Maya phones them.
3. **Qualification.** Maya asks one question at a time: land or house, location, budget in naira, outright or payment plan, timeline, Nigeria or diaspora, who else decides.
4. **Scoring is code, not vibes.** Maya passes the answers to `score_lead`, and a tested, deterministic function returns the tier and the next step. The same answers always get the same tier, and every score stores its reasons.
5. **Routing.**
...
- Import the WhatsApp number (Agents → WhatsApp → Import Account) and assign Maya to it.
...
- **WhatsApp:** Meta requires the user's permission before a business can call them. The main path avoids this, because the lead calls Maya themselves.
